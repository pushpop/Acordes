...
--------------------------
Font usage:
  theme.FONTS_GL (PixelCode) -> all glyph rendering: box-drawing, bars, shapes,
                                 indicators, arrows, toggles, selectors
  theme.FONTS_UI (Silkscreen) -> all readable text: titles, labels, hints,
                                  parameter names, values, hint bars
...
def _font(variant, size):
    ...
    "glyph" (default) -> PixelCode: box-drawing, bars, shapes, indicators
    "ui"              -> Silkscreen: labels, titles, hints, readable text
    """
    if variant == "ui":
        return theme.FONTS_UI.get(size) or theme.FONTS_GL.get(size)
    return theme.FONTS_GL.get(size)
...
def box(surface, x, y, w_px, h_px, color, double=False, fill=None):
    ...
    font = theme.FONTS_GL[theme.FONT_SMALL]
    cols = max(2, w_px // cw)
    rows = max(2, h_px // ch)
...
def hint_bar(surface, hints, y=None):
    ...
    font = theme.FONTS_UI[theme.FONT_TINY]
...
def title_bar(surface, left_text, right_text=None, y=0):
    ...
    """
    font_l = theme.FONTS_UI[theme.FONT_TINY]
    font_r = theme.FONTS_UI[theme.FONT_TINY]
...
def selector(surface, x, y, value_text, color=None, size=None):
    ...
    fg    = theme.FONTS_GL[size]   # arrows are PixelCode glyphs
    fu    = theme.FONTS_UI[size]   # value text is Silkscreen
    cw    = theme.CELL_W

    arrow_l = fg.render(SYM_ARROW_L, False, color)
    value_s = fu.render(value_text,  False, color)
    arrow_r = fg.render(SYM_ARROW_R, False, color)
...
def param_row(surface, x, y, w_px, name, value_text, bar_value=None, bar_max=1.0):
    ...
    """
    font_name  = theme.FONTS_UI[theme.FONT_TINY]
    font_value = theme.FONTS_UI[theme.FONT_SMALL]
    cw = theme.CELL_W
